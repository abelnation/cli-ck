...
_.extend(Click.prototype, {
    ...
    run: function run(argv, cb) {
        ...
        try {
            ...
            if (handler && typeof handler === 'function') {
                ...
                console.log('')
                console.error('Error: no handler specified for command: ' + lastContext.getName())
            }
    ...
    getOptionNames: function getOptionNames() {
        return Object.keys(this.optNames)
    },
    ...
    getOptionalOptionNames: function getOptionalOptions() {
        return _.difference(this.getOptionNames(), Object.keys(this.requiredOpts))
    },

    getCommandNames: function getCommandNames() {
        return _.without(Object.keys(this.commands), 'help')
    },

    getDeepCommandNames: function getDeepCommandNames() {
        // base case
        if (this.getCommandNames().length === 0) {
            return [ ]
        }

        // first add our own simple sub-commands
        var result = this.getCommandNames()

        // then recursively get all deep commands of our sub-commands
        // pre-pending the sub-commands name
        _.forEach(this.getCommandNames(), _.bind(function(cmdName) {
            var cmdContext = this.commands[cmdName].getContext()
            var cmdSubCommands = cmdContext.getDeepCommandNames()

            var recDeepCommands = _.map(cmdSubCommands, function(subCommandName) {
                return cmdContext.getName() + ' ' + subCommandName
            })

            result = result.concat(recDeepCommands)
        }, this))
    ...
    getCommand: function getCommand(command) {
        ...
        _.forEach(commands, _.bind(function(token) {
            if (!(token in context.commands)) {
                throw new ClickError('Invalid command: ' + command)
            }
